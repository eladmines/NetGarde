...
def create_blocked_site_controller(blocked_site_data: BlockedSiteCreate, db: Session, service: IBlockedSiteService):
    try:
        logger.info("POST /blocked-sites - create", extra={"domain": blocked_site_data.domain})
        result = service.create_blocked_site(blocked_site_data, db)
...
def get_blocked_site_by_id_controller(blocked_site_id: int, db: Session, service: IBlockedSiteService):
    try:
        logger.info("GET /blocked-sites/{blocked_site_id} - fetch", extra={"blocked_site_id": blocked_site_id})
        blocked_site = service.get_blocked_site_by_id(blocked_site_id, db)
...
def get_blocked_site_by_domain_controller(blocked_site_domain: str, db: Session, service: IBlockedSiteService):
    try:
        logger.info("GET /blocked-sites/by-domain - fetch", extra={"domain": blocked_site_domain})
        blocked_site = service.get_blocked_site_by_domain(blocked_site_domain, db)
        if blocked_site is None:
...
def get_blocked_sites_controller(db: Session, service: IBlockedSiteService, page: int = 1, page_size: int = 10, domain_search: str = None):
    try:
        logger.info("GET /blocked-sites - fetch", extra={"page": page, "page_size": page_size, "domain_search": domain_search})
        blocked_sites, total = service.get_blocked_sites(db, page=page, page_size=page_size, domain_search=domain_search)
...
def update_blocked_site_controller(blocked_site_id: int, blocked_site_data: BlockedSiteCreate, db: Session, service: IBlockedSiteService):
    try:
        ...
        result = service.update_blocked_site(blocked_site_id, blocked_site_data, db)
...
def delete_blocked_site_controller(blocked_site_id: int, db: Session, service: IBlockedSiteService):
    try:
        logger.info("DELETE /blocked-sites/{blocked_site_id} - delete", extra={"blocked_site_id": blocked_site_id})
        result = service.delete_blocked_site(blocked_site_id, db)
...
def get_blocked_sites_counts_by_category_controller(db: Session, service: IBlockedSiteService):
    try:
        logger.info("GET /blocked-sites/counts-by-category - fetch")
        counts = service.get_blocked_sites_counts_by_category(db)
